...
public class StickBuilt {
  ...
  /*
   * simple-minded example that builds the graph: args[0] -> args[1] -> args[2]
   * -> ... -> args[args.length - 1]
   */
    ...
    if (nodes != null) {
      for (cnt = 0; cnt < nodes.length; cnt++) {
        ...
        if ((nd[crnt] = graph.findNodeByName(name)) == null) {
          ...
          /*
           * set the initial position just so they don't all draw at (0,0). The
           * display display frame has a button that takes advantage of
           * GrappaSupport.filterGraph(...) to get a better layout by letting
           * one of the graphviz layout tools such as "dot" or "neato" do the
           * layout for you.
           */
          // nd[crnt].setAttribute(GrappaConstants.POS_ATTR, ((cnt*125) +
          // ",0")); IVAN
          if (cnt % 2 == 0)
            nd[crnt].setAttribute(GrappaConstants.POS_ATTR, ((cnt * 125) + "," + (cnt * 125)));
          else
